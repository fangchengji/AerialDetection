...
def DOTA2COCOTrain(srcpath, destfile, cls_names, difficult='2'):
    ...
    with open(destfile, 'w') as f_out:
        ...
        for f in filenames:
            basename = util.custombasename(f)
            ...
            objects = util.parse_dota_keypoints(f, (height, width))
...
def DOTA2COCO_RotatedBBox(srcpath, destfile, cls_names):
    # set difficult to filter '2', '1', or do not filter, set '-1'
    imageparent = os.path.join(srcpath, 'images')
    labelparent = os.path.join(srcpath, 'labelTxt')
    ...
    with open(destfile, 'w') as f_out:
        ...
        for f in filenames:
            ...
            # annotations
            # objects = util.parse_dota_poly2(file)
            objects = util.parse_dota_rotated_bbox(f, (height, width))
            if len(objects) == 0:
                continue
            ...
            for obj in objects:
                ...
                single_obj['segmentation'] = []
                single_obj['segmentation'].append(obj['poly'])
                single_obj['visibility'] = obj['visibility']
                single_obj['rotated_bbox'] = obj['rotated_bbox']
                single_obj['iscrowd'] = 0
                xmin, ymin, xmax, ymax = min(obj['poly'][0::2]), min(obj['poly'][1::2]), \
                                         max(obj['poly'][0::2]), max(obj['poly'][1::2])
